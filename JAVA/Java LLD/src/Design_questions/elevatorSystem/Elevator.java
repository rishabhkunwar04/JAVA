...
public class Elevator {
    ...
    public Elevator(int id,int capacity){
        ...
        currentDirection=Direction.DOWN; // arbitrary initialization we will chnage it later
        currentFloor=1;
        this.requests=new ArrayList<>();

    }
    public  synchronized void addRequest(Requests request){
        if(requests.size()<capacity){
            requests.add(request);
            System.out.println("Elevator " + id + " added request: " + request);
            //notify all threads that new request added
            notifyAll();
        }
    ...
    public synchronized Requests getNextRequest() {
    ...
    public synchronized void processRequests() {
        while (true) {
            while (!requests.isEmpty()) {
                Requests request = getNextRequest();
